 ...
 *
 * <b>Note that nodes stored inside an instance of a path should not be modified, and doing so may
 * result in unexpected results.</b>
 *
 * @param <T> represents the type of nodes that connect Path#getStart() to Path#getEndI(), as well as
 *           the type of those two nodes themselves.
 */
public class Path<T> implements Iterable<Path<T>.Segment> {
    ...
    //          path.get(0) is the first step from the start point to an intermediate node, and
     ...
     *
     *  <b>Note that nodes stored inside an instance of a Segment should not be modified, and
     *  doing so may result in unexpected results.</b>
     */
